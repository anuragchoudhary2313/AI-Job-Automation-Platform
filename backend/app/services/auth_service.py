...
class AuthService:
    ...
    async def authenticate_user(self, email: str, password: str) -> User:
        ...
        if not user:
            logger.warning(
                f"Authentication failed: User not found for login identifier {email}"
            )
    ...
    async def forgot_password(
        self, email: str, frontend_url: str = "http://localhost:5173"
    ) -> None:
        ...
        to_encode = {"sub": str(user.id), "exp": expire, "type": "password_reset"}
